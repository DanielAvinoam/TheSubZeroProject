...
HKEY RegistryManager::OpenRegistryKey(HKEY hKeyRoot, const std::wstring wsSubKey) 
{
	...
	if (ERROR_SUCCESS != ::RegOpenKeyExW(hKeyRoot, wsSubKey.c_str(), 0, KEY_ALL_ACCESS, &hkResult))
		throw Win32ErrorCodeException(DEBUG_TEXT("Could not open registry key"));
...
HKEY RegistryManager::CreateRegistryKey(HKEY hKeyRoot, const std::wstring wsSubKey) 
{
	...
	if (!::InitializeSecurityDescriptor(&SD, SECURITY_DESCRIPTOR_REVISION))
		throw Win32ErrorCodeException(DEBUG_TEXT("Error initializing security descriptor"));

	if (!::SetSecurityDescriptorDacl(&SD, TRUE, pACL, FALSE))
		throw Win32ErrorCodeException(DEBUG_TEXT("Error setting security descriptor's DACL"));
	...
	if (lRes) 
		throw Win32ErrorCodeException(DEBUG_TEXT("Error creating registry key"));
